...
> Windows: `PySite.cs` (single-process supervisor + venv + reverse-proxy vhost), `omniserv pysite add/start/stop/restart/rm/pip/list`, portable CPython via astral-sh/python-build-standalone (default UA), `python` tool-service + requirement guard, "Python app" Add-site type, and a Python sidebar tab (PythonPage). Engine/CLI/GUI build clean; runtime (download + venv + run a Flask app) to be verified on a real machine.
...
### What the macOS engine added (`engine/omniserv`) — your spec to mirror

A new **`pysite`** verb parallel to `nodesite`:

```
omniserv pysite add <name> --dir <path> --port <n> [--cmd "<run command>"] [--venv yes|no] [--python 3.13]
omniserv pysite {list|start|stop|restart|status|rm|pip} <name>
```
...
1. **No Homebrew → you need a portable Python.** Mac uses `brew install python@3.13`. On Windows, do
   ...
   *embeddable* zip (no pip/venv) and avoid the full `.exe` installer (UAC + not portable).
   - ⚠️ **Watch the User-Agent 403 you already hit with `dev.mysql.com`** — download with curl's
     **default UA** (no custom `OmniServ/…` UA) to be safe; GitHub release assets are fine either way.
...
- Engine: the `pysite` block in `engine/omniserv` (`py_proc_running`, `py_python`, `py_runpath`,
...
## 5. Rich tray / menu-bar flyout (live metrics + sites + tools)  *(macOS: OmniServApp.swift MenuBarView)*  — **medium, polish**

The Windows tray (`windows/src/OmniServ.App/TrayIcon.cs`, `ShowMenu()`) is a plain Win32
`Shell_NotifyIcon` context menu with only **Open OmniServ / Start all / Stop all / Restart all / Quit**
(wired in `MainWindow.xaml.cs`). The macOS menu-bar is a rich **flyout** (`macos/Sources/OmniServ/OmniServApp.swift`
...
Mailpit) quick-open. **`windows/src/OmniServ.Core/SystemMetrics.cs` already computes the metrics** — they're
...
> `omniserv-elevate`; functional pass — add `api`+`shop` (hosts entries elevated OK), `list`, cross-site
...
> TRUSTED on all three hosts. ⚠️ One testing gotcha for future notes: `omniserv secure <name>` (bare
...
(`omniserv site subdomain {list|add|rm}`) is verified working there (add/list/rm, cross-site conflict
...
- `windows/src/OmniServ.Core/Engine.cs` — `SiteSubdomains()` / `SiteSubdomainAdd()` / `SiteSubdomainRemove()`
...
- `windows/src/OmniServ.Core/{NginxConfig,NodeSite,PySite}.cs` — pass aliases into the vhost `server_name`.
- `windows/src/OmniServ.Cli/Program.cs` — the `case "subdomain":` CLI subcommand (~L96).
- `windows/src/OmniServ.App/Views/SiteListControl.xaml{,.cs}` — the "Manage subdomains…" dialog + alias badge.

**⚠️ Windows-specific things to VERIFY (plusemon likely tested on one platform, not Windows):**
1. **It compiles.** Build `windows/OmniServ.sln` — a community PR may have C# errors that only surface on a
...
2. **Hosts-file entries (Windows has no dnsmasq).** Add calls `EnsureHosts(host)`; remove calls
   ...
   `omniserv-elevate.exe` actually implements a `hosts-remove` verb** — if it doesn't, removing a subdomain
...
The **same PR #3** added the Linux GTK side, and the shared `engine/omniserv` gives Linux the
`omniserv site subdomain {list|add|rm}` CLI **for free** (already verified working on macOS — same engine).
So on Linux you only verify the **GUI + a functional pass**, then ship:
- `linux/app/omniserv/pages.py` — `site_subdomains(win, s)` (Adw.MessageDialog add/remove, ~L188), the
...
- **Also ship @plusemon's PR #2** (Linux-only, already merged): the **"Open nginx/Apache config"** context
  item in `pages.py` (~L153) — `os` is imported, path is `~/.omniserv/{nginx|apache}/sites/<name>.conf`,
...
`AH00558` FQDN warning). OmniServ's engine probed every service with `--version` first, so **every
api/dashboard refresh spawned a rogue Apache** with the wrong config; the strays then held :8080 so
OmniServ's real Apache couldn't start, and the GUI showed the AH00558 text as the "version". Same Apache
