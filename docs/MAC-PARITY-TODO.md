...
review** — Emon Khan (rightly) flagged it. I did the audit retroactively and it came back **clean**, but
...
> ⚠️ Also: the `macos/Sources/OmniServ` code from plusemon's PR #3 (AppState/Models/WebsitesPanel
...
your side added the v1.7.7/v1.7.8 ionCube work (`engine/omniserv` + `macos/` build scripts + docs);
...
target master, which lacked all my work. Emon Khan approved converging on ONE branch.
...
1. Merged `origin/master` → `windows-port-cli`. Almost everything auto-merged because we changed
   disjoint files. **Your `engine/omniserv` came through byte-identical to master's tip — I have made
...
2. **One conflict**, `linux/engine/DELTAS.md`, and it was a happy one: your v1.7.7 commit added a
   ...
   solved via the DISTRO conf.d (`00-omniserv-ioncube.ini` sorts before `10-opcache.ini`; on Debian
...
**Heads-up — `master` is now protected (2026-07-22).** Emon Khan enabled a branch ruleset
...
(e.g. scrubbing a leaked secret), Emon Khan must temporarily disable the ruleset in
...
> in `engine/omniserv`. The Mac's only direct `curl` fetches are WordPress / WP-salts / ionCube /
...
> Mac enables ionCube via a **static ini** (`~/.omniserv/php/conf.d/<ver>/00-ioncube.ini` →
...
> **loaded on php@8.1** (`omniserv php status`). So on a Mac reboot, autostart's FPM start loads the loader
...
> window. **#9/#9b — per-site SSL Install/Reinstall/Remove:** the shared `engine/omniserv` already has
...
> `engine/omniserv` (the "🎉 Congratulations! Your website is live now!" page — no web-server name, no
...
Two features landed on the Windows build that the macOS build (engine `engine/omniserv` + the
...
**Problem:** users skip the readme, install OmniServ, and immediately add a site with **nothing
...
- `windows/src/OmniServ.Core/Engine.cs`:
...
- `windows/src/OmniServ.App/Views/SitesPage.xaml.cs` → `Add_Click` calls `EnsureRequirements(...)` before
  creating the site (returns false to abort); the dialog lists missing pieces, installs on confirm.
- `windows/src/OmniServ.App/MainWindow.xaml.cs` → `FirstRunThenUpdateCheck()` (waits for XamlRoot, then
...
- When ticked, after creating the site, call the engine's existing **`omniserv secure <name>.<tld>`**
...
**Windows source to diff against**
- `windows/src/OmniServ.App/Views/SitesPage.xaml` — `<CheckBox x:Name="SslBox" Content="HTTPS" IsChecked="True">` in the add-site bar.
- `windows/src/OmniServ.App/Views/SitesPage.xaml.cs`:
...
- `windows/src/OmniServ.Core/Engine.cs` → `Secure(string domain)` — added `Ok($"secured: https://{domain}")`
...
**Windows source to diff against**
- `windows/src/OmniServ.App/MainWindow.xaml.cs`:
...
- `windows/src/OmniServ.App/Services/Updater.cs` — `Check()` → `record Result(bool UpdateAvailable,
...
- `windows/src/OmniServ.Core/Config.cs` — `auto_update` bool, **default `true`** (the gate).
...
Forbidden` for a custom `User-Agent`** (Windows sent `OmniServ/0.1 (+github…)`; browser UAs also 403),
...
1. Grep the engine: `grep -nE 'mysql|dev\.mysql\.com|brew install mysql' engine/omniserv`.
...
2. Reproduce (any shell): `curl -fIL -A "OmniServ/…"  '<the mysql zip url>'` vs `curl -fL -o /dev/null '<url>'`
...
**Windows source to diff against:** `windows/src/OmniServ.Core/Downloader.cs` — `CurlTo(url, dest,
...
**Check on macOS — `engine/omniserv` injects salts via `awk -v s="$salts" '…{printf "%s", s}…'`:**
...
server + document root + "replace this file" hint + emon.bd link; `phpinfo()` behind
`?phpinfo=1`). **The PHP/HTML file is identical cross-platform — copy it verbatim from Windows.**

**Windows source to diff against**
- Editor/Terminal: `windows/src/OmniServ.App/Views/SiteListControl.xaml` (two `MenuFlyoutItem`s) +
  `SiteListControl.xaml.cs` → `CodeEditor_Click` / `Terminal_Click` + `FindEditor()` / `OnPath()`.
- Default page: `windows/src/OmniServ.Core/Engine.cs` → `const string DefaultIndexPhp` (the whole page)
...
static-php-cli (`dl.static-php.dev`) — installed to `/usr/local/lib/omniserv/php/<v>/php-fpm` and
...
## L2. Linux-only — `omniserv self-update` CLI  *(Linux: linux-v1.0.16)*
...
`omniserv self-update` verb: queries the GitHub releases API for the newest `linux-v*` `.deb`, compares
...
a CLI `omniserv self-update` is desired on macOS it would fetch the newest `v1.7.x` `.dmg`/zip and
...
**A. Default site page — ALREADY DONE for macOS (shared engine).** The `engine/omniserv` default
...
OmniServ · emon.bd" line per request). So the Mac already ships this via the shared engine — no
action needed. (Windows has its own copy in `OmniServ.Core/Engine.cs::DefaultIndexPhp`, updated to match.)
...
**Windows source to diff against:** `windows/src/OmniServ.App/Views/SitesPage.xaml` (ComboBox labels +
`Tag` for the real value) + `SitesPage.xaml.cs` (`SelectedServer` reads `Tag`); `OmniServ.Core/Engine.cs`
`RequiredServices` (nginx+apache for Apache) + `DefaultIndexPhp` (generic page). Linux equivalent:
`linux/app/omniserv/window.py` (descriptive DropDown strings + tooltip, value stays index-mapped).
...
- **Sidebar structure.** App icon + "OmniServ" name pinned at the sidebar **top**, nav items in the
...
- **Tool auto-HTTPS.** `pma_install`/`adminer_install`/`mailpit_setup` now call a shared `tool_autosecure`
  → `omniserv secure <tool>.test` (best-effort, when mkcert is installed) so the tools work over https out
...
- **`cmd_secure` proxy-vhost fix.** Securing a **proxy** vhost (Mailpit) used to re-render it as a *php*
  ...
  a real Mac bug too** — `omniserv secure mailpit.test` on macOS would have broken the Mailpit vhost.
...
- **phpMyAdmin mysqli** *(finalized in linux-v1.0.26)*. The portable static **`common`** build ships
  ...
  depend on a non-default one the user may not have installed). Verified in WSL that OmniServ's
...
- **dnsmasq shows active once installed** (Ubuntu ships dnsmasq-base with no running unit; OmniServ-Linux
...
re-render + full nginx stop/start). CLI verbs: `omniserv unsecure <domain>` / `omniserv resecure <domain>`.
...
in the shared `engine/omniserv` (delete certs + re-render vhost + restart; reuse cmd_secure's proxy-aware
...
**Shared engine (macOS gets these FREE — verify):** `engine/omniserv` now has `cmd_unsecure` +
`cmd_resecure` (dispatched as `omniserv unsecure|resecure <domain>`) + a factored `_rerender_site_vhost`
...
**✅ REAL FIX (linux-v1.0.40):** the actual cause was NOT the async race (that was hardening) — `engine.py` `_PRIVILEGED` listed `secure` but not `unsecure`/`resecure`, so removal ran UNPRIVILEGED (no pkexec prompt) and couldn't restart nginx. Added both to `_PRIVILEGED`. Emon Khan diagnosed it from "install prompts for a password, remove doesn't"; my WSL tests missed it (passwordless sudo masks the privilege gap). **Mac already runs its SSL apply via a privileged restart, so it was never hit.**
...
**macOS TODO:** add the same three items to the Websites-panel row menu (Reinstall → `omniserv resecure`,
Remove → `omniserv unsecure`) — the engine verbs already exist. Also confirm the multi-label fix (a Mac
...
- Removed the Scheduled-Task restart machinery (OmniServIonRestart) and the dashboard "Enable ionCube"
...
`opcache.jit = tracing` (OmniServ's own php.ini tuning) crashed PHP 8.4 php-cgi workers with
...
SDK. Laragon + XAMPP ship a cacert.pem; now OmniServ does too: `EnsureCaBundle` downloads the Mozilla
...
## win-v1.0.60 (part 2) — ✅ DONE on macOS (v1.7.11) — pin PHP sessions/uploads/temp to a writable OmniServ dir
...
`upload_tmp_dir` and `sys_temp_dir` to OmniServ's tmp in every php.ini. Site-level fix for imports:
...
OmniServ-writable dir (brew PHP defaults may rely on /var/tmp — usually fine on mac, but pinning to
~/.omniserv/tmp is the same robustness win, and imported-site `.user.ini` Linux paths can bite there
...
- **⚠️ SUPERSEDES the 1.0.48 loginitem note above.** Unprivileged `systemctl --user` start-at-login was
  ...
  presumably already has: a SYSTEM unit (`/etc/systemd/system/omniserv.service`, WantedBy=multi-user)
  runs `start all` as root at boot with `Environment=OMNISERV_OWNER_UID=<uid>` (the engine's elevation
  block falls back to it when PKEXEC_UID/SUDO_UID are absent, so workers/sockets still drop to the
  desktop user — NOT root), plus a per-user autostart .desktop launching `omniserv-gui --background`
...
**This was a WINDOWS-ONLY gap. VERIFIED (2026-07-28): the SHARED `render_fpm_pool` (engine/omniserv:1011)
...
> (`~/OmniServ/www/My Spaced Site`): (1) renders **quoted**; (2) `nginx -t` **valid**; (3) round-trip
...
**macOS `swift build`** (on GitHub's `macos-latest` runner) · **Windows `dotnet build OmniServ.sln`** ·
**Linux** `compileall` + `ruff --select F821,F822` (undefined names — the class `py_compile` can't
see, because it only fails at runtime) + `bash -n` over `engine/omniserv` and every shipped `.sh`.
