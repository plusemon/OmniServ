# OmniServ for Windows — Port Plan & Architecture Spec
...
- **Core logic:** a shared **`OmniServ.Core`** class library (the brains).
- **CLI:** a thin **`omniserv.exe`** console app over `OmniServ.Core` — mirrors the
  Mac's "transparent CLI you can run standalone." The WinUI app calls into
  `OmniServ.Core` directly (in-process), NOT by shelling out to the CLI.
- **Installer:** **Inno Setup** (or WiX/MSI) producing a branded `OmniServ-Setup.exe`.
...
windows/
  OmniServ.sln
  src/
    OmniServ.Core/        # services model, vhost render, php-cgi mgr, certs, dns, db
    OmniServ.Cli/         # omniserv.exe  (doctor/init/install/site/db/start/stop/…)
    OmniServ.App/         # WinUI 3 GUI  (Dashboard/Services/Sites/DB/Node/Logs/Settings)
  installer/
    omniserv.iss          # Inno Setup script (branded, Emon Khan blue #0d6efd)
...
| Engine | `engine/omniserv` (bash) | `OmniServ.Core` (C#) + `omniserv.exe` |
| Pkg mgr | Homebrew (`brew install`) | **download pinned portable zips** into the data dir (see §3) |
| Data dir | `~/.omniserv` | `%LOCALAPPDATA%\OmniServ` |
| Binaries | brew kegs in `/opt/homebrew` | `%LOCALAPPDATA%\OmniServ\bin\<tool>\<ver>` |
...
| Process mgmt | launchd-free, raw `Process` + pid files | child `Process` + pid/port files in `…\OmniServ\run` |
...
| Elevation | `osascript … with administrator privileges` (sudo) | UAC: a manifested **`omniserv-elevate.exe`** helper for hosts-file/firewall writes |
...
3. Track pid + port in `…\OmniServ\run\php-<ver>.json`. "Running" = pid alive.
...
demand into `…\OmniServ\bin` — the Windows equivalent of `brew install`. Sources:
...
## 3. Feature-parity checklist (map to current `omniserv` subcommands)

Port each Mac engine verb to a `OmniServ.Core` method + a `omniserv.exe` subcommand:

- [ ] `doctor` — check deps/ports/conflicts (also detect Laragon/XAMPP on 80/443)
- [ ] `init` — create `%LOCALAPPDATA%\OmniServ\{config,nginx\sites,bin,run,logs,sites,certs,tmp}`
...
Pattern: a tiny **`omniserv-elevate.exe`** with `requireAdministrator` in its manifest
...
# release
.\build.ps1            # dotnet publish (win-x64, self-contained) + iscc → installer\dist\OmniServ-Setup.exe
```

Then cut the GitHub release with `OmniServ-Setup.exe` so the in-app updater finds it.
