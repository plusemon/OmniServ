...
- Emon Khan reviews/merges PRs (or merge once approved). Small follow-ups (e.g. the two
...
2. `windows/installer/omniserv.iss` → `#define MyAppVersion "0.2.0"`
...
# → windows\installer\dist\OmniServ-Setup-<ver>.exe
```

Smoke-test the `.exe` on a **clean** Windows user/VM: install → launch → `omniserv init` →
...
gh release create win-v0.2.0 `
  windows\installer\dist\OmniServ-Setup-0.2.0.exe `
  --prerelease `
  --title "OmniServ for Windows 0.2.0 (preview)" `
...
## E. The Windows in-app updater (`OmniServ.App/Services/Updater.cs`)
...
(Mirror the Mac flow in `macos/Sources/OmniServ/AppState.swift` `checkForUpdate` / `downloadAndInstall`,
...
- Assets: `OmniServ-<ver>.pkg` (mac) + `OmniServ-<ver>.dmg` (mac) + `OmniServ-Setup-<ver>.exe` (win) [+ `.deb`/`.AppImage` later].
...
- [ ] Version bumped in **both** `Directory.Build.props` and `omniserv.iss`
- [ ] `build.ps1` produced `OmniServ-Setup-<ver>.exe`; installed + smoke-tested on a clean Windows
