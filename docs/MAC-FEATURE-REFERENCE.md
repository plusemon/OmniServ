# OmniServ macOS — Feature & GUI Reference (parity checklist for the Windows/Linux ports)
...
>
> Mac source lives in `macos/Sources/OmniServ/`. The engine verbs are in `engine/omniserv`.
> Windows equivalents live in `windows/src/OmniServ.App/` (GUI) + `OmniServ.Core` (engine).

---

## 0. App shell & menu-bar behavior — `OmniServApp.swift`, `ContentView.swift`

**Mac files:** `OmniServApp.swift` (menu-bar app, on-demand window, MenuBarView), `ContentView.swift` (sidebar + routing + StatusFooter).
...
- **Add Site sheet** (`AddSiteSheet`): name + `.tld`; **Type = WordPress / PHP / Others(static) / Node app**;
  **PHP version**; **web server** (nginx/apache); **site root = default folder or custom (folder picker)**.
  ...
  - *Node app* → frontend (folder + run command + port) **+ optional backend/API** (folder + command + port) +
    api-paths regex. OmniServ runs both as supervised processes and reverse-proxies them at the domain.
...
A first-class **Node** site type. Engine: `omniserv nodesite {add|list|rm|start|stop|restart|status|npm}` —
...
→ BE) with mkcert HTTPS. Config in `~/.omniserv/node-sites/<name>.json`; the `api` snapshot carries
...
- **Startup:** **Launch at login** · **Start services when OmniServ launches** · **password-less control** helper
...
> `macos/Sources/OmniServ/*.swift`** file and the matching **`windows/src/OmniServ.App/Views/*Page.xaml`**, then add
> any missing controls/cards/actions and match the layout order. The **engine** side (`OmniServ.Core`) already
