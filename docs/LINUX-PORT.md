...
**Linux can reuse most of the existing `engine/omniserv` bash script.** bash, php-fpm,
...
- **Engine:** **reuse `engine/omniserv`** (bash) with a thin platform-delta layer.
...
- **GUI:** **GTK4 + libadwaita** — native GNOME/Ubuntu look, the natural Linux
  ...
  - **Language sub-choice (decide when you start):**
    - **Rust + `gtk4-rs`** — single self-contained binary, matches OmniServ's
...
linux/
  ...
  packaging/
    deb/               # debian/ control, rules → omniserv_<ver>_amd64.deb
    appimage/          # AppDir + appimagetool recipe
  build.sh             # build app + .deb + .AppImage
  README.md
```

(Or: keep using the repo-root `engine/omniserv` directly and add only a small
...
|---|---|---|
| Engine | `engine/omniserv` (bash) | **same bash script** + platform deltas |
...
| Data dir | `~/.omniserv` | `~/.omniserv` (unchanged — already `$HOME`-based) |
...
| Elevation | `osascript … administrator` / sudoers | **pkexec** (PolicyKit GUI prompt) OR `/etc/sudoers.d/omniserv` (the engine's `helper` already writes this!) |
...
## 4. Feature-parity checklist (same `omniserv` verbs)
...
- [ ] `init` — unchanged (`~/.omniserv` skeleton)
...
- [ ] `helper {install|uninstall}` — **delta:** writes `/etc/sudoers.d/omniserv`
...
2. **systemd-resolved drop-in** — run dnsmasq on a non-conflicting address and add
   `/etc/systemd/resolved.conf.d/omniserv.conf` (or an `~test` routing domain via
   `resolvectl`) pointing `.test` at it. True wildcard, no :53 fight.
3. **NetworkManager's dnsmasq** — add `/etc/NetworkManager/dnsmasq.d/omniserv.conf`
...
- **pkexec** (PolicyKit) → a graphical password prompt, the direct analog of macOS
  ...
  `pkexec /path/to/omniserv <verb>`.
- **`/etc/sudoers.d/omniserv`** → password-less specific commands, the same
...
- **systemd user service:** `~/.config/systemd/user/omniserv.service`
  (`ExecStart=…/omniserv start all`), `systemctl --user enable omniserv`, plus
  `loginctl enable-linger $USER` so it can run pre-login if wanted. The clean analog
  of the LaunchAgent.
- Or a desktop-autostart entry: `~/.config/autostart/omniserv.desktop`.
...
../engine/omniserv doctor
../engine/omniserv init
../engine/omniserv site add myapp --php 8.4

# GUI (Rust path):
cd app && cargo run            # gtk4-rs
# GUI (Python path):
python3 -m omniserv_app         # PyGObject

# package:
./build.sh                     # → packaging/dist/omniserv_<ver>_amd64.deb + OmniServ-<ver>.AppImage
```
...
Mac), nginx/apache vhosts, the branded default page + WP-salts logic, `~/.omniserv` layout, the `api`
JSON shape.
