        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if !pyInstalled {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("OmniServ runs Python apps with a managed Python (Homebrew python@3.13). Install it once — each app then gets its own virtualenv for its dependencies.")
                ...
                ManagedAppsSection(
                    title: "Python apps",
                    apps: pyApps,
                    emptyText: "No Python apps yet. Add one — Flask, Django, FastAPI, Gunicorn or Uvicorn. OmniServ supervises the process and serves it at name.\(state.snapshot?.config.tld ?? "test") over HTTPS.",
