...
struct AddSiteSheet: View {
    ...
    private var defaultRoot: String {
        (state.snapshot?.config.sitesRoot ?? "~/OmniServ/www") + "/" + (cleanName.isEmpty ? "<name>" : cleanName)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if isNode {
                Text("A managed Node app: a frontend (e.g. Next.js) plus an optional backend/API (e.g. Laravel). OmniServ runs both and reverse-proxies them at the domain.")
                ...
                Text("A managed Python web app (Flask / Django / FastAPI / Gunicorn / Uvicorn). OmniServ supervises the process on a port and reverse-proxies it at the domain. Your run command can use $PORT.")
                    .font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
                HStack {
                ...
                if server == "apache" {
                    Text("Apache runs behind nginx for native .htaccess support — OmniServ will use both.")
                        .font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
                }
