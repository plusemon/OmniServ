...
struct OmniServApp: App {
    ...
    var body: some Scene {
        ...
        MenuBarExtra("OmniServ", systemImage: "server.rack") {
...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if !state.activeSites.isEmpty {
                ...
                HStack {
                    ...
                    if state.activeSites.count > 5 {
                        Text("\(state.activeSites.count) running — Open OmniServ for all")
            ...
            Divider()
            HStack {
                Button("Open OmniServ") { DashboardWindow.shared.show() }
                Spacer()
