...
enum SidebarItem: String, CaseIterable, Identifiable {
    case dashboard, services, sites, databases, node, python, logs, settings
    ...
    private var mainView: some View {
        ...
        .alert("Update available", isPresented: Binding(
            ...
            if case .available(let v, _) = state.updateStatus {
                Text("OmniServ \(v) is available. Update now? (Settings ▸ Updates anytime.)")
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            HStack(spacing: 4) {
                ...
                Text("OmniServ v\(state.appVersion)").font(.caption2)
                Spacer()
            }
