...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Updates") {
                ...
                case .available(let version, let pkg):
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("Downloads the new installer, opens it, and quits OmniServ so it can update. Reopen when done.")
                            .font(.caption).foregroundStyle(.secondary)
                    }
                case .failed(let msg):
                    HStack {
            ...
            Section("Startup") {
                Toggle("Launch OmniServ at login", isOn: Binding(
                    get: { state.loginItemEnabled },
                    set: { v in Task { await state.setLoginItem(v) } }))
                Toggle("Start services when OmniServ launches", isOn: Binding(
                ...
                if state.helperInstalled {
                    ...
                    Text("nginx binds :80/:443 (root), so Start/Stop asks for your password each time. This installs a one-time sudoers rule so it never asks again — and lets OmniServ auto-start at login without a prompt.")
                        .font(.caption).foregroundStyle(.secondary)
                }
            }
            Section("List sizes") {
            ...
            Section("Defaults for new sites") {
                ...
                LabeledContent("Sites root") {
                    TextField("~/OmniServ/www", text: $sitesRoot)
                }
            }
            Section {
                HStack {
        ...
        .confirmationDialog("Update OmniServ to v\(confirmUpdate?.version ?? "")?",
            ...
            Text("OmniServ will download the installer, open it, and quit so it can update.")
        }
    }
