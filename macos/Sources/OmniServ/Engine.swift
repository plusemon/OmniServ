...
/// Thin wrapper around the `omniserv` bash engine. Sendable so it can run off the
...
final class Engine: Sendable {
    ...
    /// password via $OMNISERV_DB_PASSWORD so it never appears in `ps`/argv).
    @discardableResult
    func run(_ args: [String], env: [String: String] = [:]) throws -> String {
        ...
        let q = DispatchQueue(label: "omniserv.engine.io", attributes: .concurrent)
