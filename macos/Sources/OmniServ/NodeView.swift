            ...
            VStack(alignment: .leading, spacing: 22) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if !state.fnmInstalled {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("OmniServ manages Node versions with fnm. Install it to add Node versions (Node apps below run with the default version).")
                ...
                ManagedAppsSection(
                    title: "Node apps",
                    apps: nodeApps,
                    emptyText: "No Node apps yet. Add one — a frontend (e.g. Next.js) plus an optional backend/API (e.g. Laravel). OmniServ runs both and serves them at name.\(state.snapshot?.config.tld ?? "test").",
