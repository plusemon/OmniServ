...
final class AppState {
    ...
    /// OmniServ runs persistently (menu bar), so a one-shot launch check misses releases
    ...
    private func retireOldSMAgents() {
        ...
        try? SMAppService.agent(plistName: "com.emon.omniserv.helper.plist").unregister()
        d.set(true, forKey: "retiredSMAgents")
    }

    /// Find the omniserv engine: explicit env override, then the user's config
    /// root, then the dev checkout. (Phase 5 will bundle it inside the .app.)
    static func resolveEnginePath() -> String {
        if let e = ProcessInfo.processInfo.environment["OMNISERV_ENGINE"], !e.isEmpty { return e }
        var candidates: [String] = []
        // bundled engine inside OmniServ.app/Contents/Resources (self-contained app)
        if let res = Bundle.main.resourceURL?.appendingPathComponent("omniserv").path { candidates.append(res) }
        candidates += [
            "\(NSHomeDirectory())/.omniserv/engine/omniserv",
            "/Applications/ServBay/www/OmniServ/engine/omniserv",
        ]
    ...
    /// OmniServ-managed tool sites — not real websites, hidden from the site lists.
    ...
    func reload() async {
        let eng = engine
        // make sure ~/.omniserv exists (idempotent; needs no Homebrew) so `api` works
    ...
    func setRootPassword(_ pw: String) async {
        let env = pw.isEmpty ? [:] : ["OMNISERV_DB_PASSWORD": pw]
    ...
    func createDatabase(_ name: String, engine dbEngine: String, password: String = "") async {
        ...
        let env = password.isEmpty ? [:] : ["OMNISERV_DB_PASSWORD": password]
    ...
    func setDatabasePassword(_ name: String, engine dbEngine: String, password: String) async {
        ...
        await runUser(["db", "passwd", name, "--engine", dbEngine],
                      ...
                      env: ["OMNISERV_DB_PASSWORD": password])
        await reloadDatabases()
    }
    ...
    private func announceUpdate(_ version: String) {
        ...
        if NSApp.windows.allSatisfy({ !$0.isVisible }) {
            ...
            c.requestAuthorization(options: [.alert, .sound]) { granted, _ in
                ...
                content.title = "OmniServ update available"
                content.body = "Version \(version) is ready — open OmniServ to update."
                c.add(UNNotificationRequest(identifier: "omniserv-update", content: content, trigger: nil))
            }
        }
    }
    ...
    /// /Applications/OmniServ.app. User clicks through (one password) and reopens.
    func downloadAndInstall(_ pkgURL: String) async {
        ...
        do {
            ...
            let dest = FileManager.default.temporaryDirectory.appendingPathComponent("OmniServ-update.pkg")
