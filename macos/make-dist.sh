#!/bin/bash
# Build distributables: OmniServ.app → OmniServ-<ver>.dmg (drag install) + .pkg (installer).
# Ad-hoc signed (local). Run: ./make-dist.sh
set -euo pipefail
cd "$(dirname "$0")"

APP_NAME="OmniServ"
VERSION="${1:-${VERSION:-1.0.4}}"          # keep in sync with build-app.sh
...
<style>
  ...
  <div class="brand">Emon Khan</div>
  <h1>OmniServ $VERSION</h1>
  <p class="tag">Your own free local web-server for macOS — an alternative to ServBay / Herd.</p>
  <p style="font-size:13px;margin:0 0 6px">This installer will place <b>OmniServ</b> in your Applications folder. It includes:</p>
  <ul>
  ...
  <div class="free">100% free &amp; open-source — built with ❤️ by <b>Emon Khan</b> (Emon Khan).</div>
</body></html>
HTML
...
# Drop the build artifact so it doesn't show up as a second OmniServ.app in
# Spotlight/Launchpad — the .dmg and .pkg already contain it.
rm -rf "$APP"

echo
echo "Distributables in $PWD/$DIST:"
echo "  • $APP_NAME-$VERSION.dmg   — open, drag OmniServ to Applications"
