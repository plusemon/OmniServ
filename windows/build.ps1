# OmniServ (Windows) - build distributables.
#
...
#                            # OmniServ fetches/extracts via Windows' SIGNED curl.exe + tar.exe
#                            # (so omniserv.exe is never the process that drops executables).
...
dotnet publish src/OmniServ.App/OmniServ.App.csproj -c $Configuration -r $Rid --self-contained true -p:WindowsAppSDKSelfContained=true "-p:Version=$ver" -o $publish
dotnet publish src/OmniServ.Cli/OmniServ.Cli.csproj -c $Configuration -r $Rid --self-contained true "-p:Version=$ver" -o $publish
dotnet publish src/OmniServ.Elevate/OmniServ.Elevate.csproj -c $Configuration -r $Rid --self-contained true "-p:Version=$ver" -o $publish
...
if ($Bundle) {
    ...
    # Use the freshly-published CLI to populate payload\bin via OMNISERV_HOME.
    $cli = Join-Path $publish "omniserv.exe"
    $env:OMNISERV_HOME = $payload
    & $cli init | Out-Null
