...
OmniServ for Windows is **open-source and unsigned** (no paid code-signing certificate).
...
When you run `OmniServ-Setup-0.1.0.exe`, Windows SmartScreen may show a blue "Windows
...
OmniServ is a *local web-server manager*: it downloads server binaries (nginx, PHP, MySQL,
...
executables** as a generic "dropper" (e.g. `Win64/Agent_AGen`). OmniServ does exactly that —
legitimately — so it can trip that heuristic and the app may get deleted/quarantined.

OmniServ already minimizes this: every download is performed by Windows' own **signed**
...
1. **Restore the app if it was already quarantined.**
   - ESET: **Tools → Quarantine** → select `OmniServ.App.exe` → **Restore**. (Or just re-run the installer.)
2. **Add exclusions** so it won't be flagged again. Exclude these two folders:
   - `C:\Program Files\OmniServ`
   - `%LOCALAPPDATA%\OmniServ`  (i.e. `C:\Users\<you>\AppData\Local\OmniServ`)
   ...
   *Manage settings* → *Exclusions* → **Add an exclusion → Folder** → pick each folder.
   ...
   ```powershell
   Add-MpPreference -ExclusionPath "$env:ProgramFiles\OmniServ", "$env:LOCALAPPDATA\OmniServ"
   ```
3. Launch OmniServ and install the services you need — it will work normally.
