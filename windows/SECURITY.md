# Security policy & threat model — OmniServ for Windows

OmniServ is a **local development** web stack. It is meant to run on a developer's own
...
**Everything binds to loopback (`127.0.0.1`) only.** No OmniServ-managed service listens on
...
users with passwords are available (`omniserv db create <name> <password>`).
...
- **The elevated helper** (`omniserv-elevate.exe`, used only for hosts-file edits + the mkcert
...
`bdemon00 at gmail dot com` (or open a GitHub security advisory) with steps to
...
**remote/LAN** actor to reach a OmniServ service. Those are taken seriously and fixed quickly.
