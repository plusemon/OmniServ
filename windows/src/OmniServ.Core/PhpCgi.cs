...
public static class PhpCgi
{
    ...
    /// <summary>OmniServ-managed extra-ini dir for a version (ionCube etc.), loaded via PHP_INI_SCAN_DIR.</summary>
    ...
    public static bool Start(string version)
    {
        ...
        // in-process spawn produces php that loads ionCube. Meanwhile the child-omniserv.exe path proved
    ...
    /// (omniserv.exe) — see Start(). After binding, when ionCube is configured for this version we
    ...
    /// <summary>OmniServ's php.ini defaults — generous uploads + performance (OPcache/JIT/realpath).</summary>
    ...
    /// <summary>Apply OmniServ's limits + performance directives to a build's php.ini (active or
    ...
    private static void EnsureLimits(string phpDir)
    {
        try
        {
            ...
            // Pin sessions/uploads/temp to a guaranteed-writable OmniServ dir. Unset, PHP falls back
