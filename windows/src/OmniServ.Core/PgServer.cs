...
/// <summary>
/// OmniServ's own PostgreSQL server on Windows — a fresh data dir under
/// <c>%LOCALAPPDATA%\OmniServ\pgdata</c>, initialized with <c>--auth=trust</c> (passwordless
...
public static class PgDatabase
{
    ...
    {
        ValidName(name);
        if (!PgServer.Running()) throw new BhException("PostgreSQL not running — omniserv start postgresql");
