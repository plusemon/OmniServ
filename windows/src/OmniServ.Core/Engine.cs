...
namespace OmniServ.Core;

/// <summary>
/// The brains — the C# analog of the mac <c>engine/omniserv</c> script. The WinUI
/// app calls these methods in-process; <c>omniserv.exe</c> exposes the same surface
/// on the command line.
/// </summary>
public sealed class Engine
{
    ...
    private static void NeedInit()
    {
        if (!Directory.Exists(Paths.Config))
            throw new BhException("not initialized — run: omniserv init");
    }

    // ── init ────────────────────────────────────────────────────────────────────
    public void Init()
    {
        Hdr($"Initializing OmniServ at {Paths.Home}");
    ...
    public void Install(string tool)
    {
        ...
        if (tool == "all")
        {
            // The core stack — everything OmniServ needs to serve a PHP site, self-contained.
    ...
    /// <summary>The core stack OmniServ needs before any site works (used by the first-run setup prompt):
    ...
    {
        NeedInit();
        if (string.IsNullOrEmpty(tool)) throw new BhException("usage: omniserv update <tool>");
        try { Stop(tool); } catch { }
    ...
    {
        NeedInit();
        if (string.IsNullOrEmpty(tool)) throw new BhException("usage: omniserv uninstall <tool>");
        var cfg = Config.Load();
        try { Stop(tool); } catch { }
    ...
    public void Start(string svc)
    {
        ...
        if (svc is "all" or "")
        {
            ...
            foreach (var v in ActivePhpVersions(cfg))
            {
                try
                {
                    ...
                    else Warn($"php {v} not installed — omniserv install php@{v}");
                }
        ...
        if (svc == "redis")     { if (Redis.Start()) Ok($"redis on :{Redis.Port}"); else throw new BhException("redis not installed — omniserv install redis"); return; }
        if (svc == "memcached") { if (Memcached.Start()) Ok($"memcached on :{Memcached.Port}"); else throw new BhException("memcached not installed — omniserv install memcached"); return; }
        if (svc == "mailpit") { if (MailpitServer.Start()) Ok($"mailpit on UI :{MailpitServer.UiPort} / SMTP :{MailpitServer.SmtpPort}"); else throw new BhException("mailpit not installed — omniserv mailpit"); return; }
    ...
    /// Run from the CLI (`omniserv __heal-php`) — the App schedules it 90s + 5min after launch so a
    ...
    public void PhpHealPass()
    {
        ...
        using var mutex = new System.Threading.Mutex(false, "OmniServPhpHealPass");
    ...
    {
        NeedInit();
        using var mutex = new System.Threading.Mutex(false, "OmniServPhpHealLoop");
    ...
    private const string DefaultIndexPhp = @"<?php // OmniServ default page - replace this index.php with your own site. ?>
<!doctype html>
<html lang='en'>
<head>
<meta charset='utf-8'>
...
";
    ...
    {
        NeedInit();
        if (string.IsNullOrWhiteSpace(name)) throw new BhException("usage: omniserv site add <name> [--php 8.4] [--root path]");
        // Allow dots for multi-label local domains (e.g. my.emon -> my.emon.test),
        ...
        else Warn($"php {version} not installed — omniserv install php@{version} (site will 502 until then)");
    ...
    private void Provision(string name, string type, string root)
    {
        ...
        // installing it on demand (a fresh OmniServ has no services yet).
    ...
    {
        NeedInit();
        if (server is not ("nginx" or "apache")) throw new BhException("usage: omniserv site server <name> <nginx|apache>");
        var cfg = Config.Load();
        var conf = Path.Combine(Paths.NginxSites, $"{name}.conf");
        if (!File.Exists(conf)) throw new BhException($"no such site: {name}");
    ...
    private void RenderSite(string name, string domain, string root, string phpKey, string server, Config cfg, IEnumerable<string>? aliases = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(root))
        {
            ...
            else throw new BhException($"site '{name}' has no document root — fix it with:  omniserv site root {name} <folder>");
        }
    ...
    {
        NeedInit();
        var mkc = Tools.MkcertExe() ?? throw new BhException("mkcert not installed — run: omniserv install mkcert");
    ...
    public void Status()
    {
        ...
        Hdr("Sites");
        if (snap.Sites.Count == 0) Info("no sites yet — omniserv site add <name>");
    ...
    private void EnsureHosts(string domain)
    {
        ...
        if (Environment.GetEnvironmentVariable("OMNISERV_SKIP_HOSTS") is { Length: > 0 })
        {
            Warn($"hosts skipped (OMNISERV_SKIP_HOSTS) — add manually: 127.0.0.1 {domain}");
            return;
        }
    ...
    private void EnsureMkcertCa(string mkc)
    {
        ...
        if (Elevation.Run("mkcert-install")) Ok("mkcert CA installed (browsers + AV scanners will trust OmniServ certs)");
    ...
    private void ReissueSiteCertificateIfSecure(string conf, string domain)
    {
        ...
        if (mkc is null) { Warn($"mkcert not installed — HTTPS aliases need: omniserv resecure {domain}"); return; }
        Directory.CreateDirectory(Paths.Certs);
    ...
    private void Tool(string label, string? path)
    {
        ...
        else Warn($"{label,-12} not found (install via omniserv install / the GUI)");
    }
    ...
    public void ConfigSet(string key, string val)
    {
        ...
        if (key is "http_port" or "https_port" or "tld")
        {
            RegenVhosts(cfg);
            NginxConfig.RenderMain(cfg);
            Warn("restart nginx to apply: omniserv restart nginx");
            if (key == "tld") Warn($"TLD changed — re-issue HTTPS per site: omniserv secure <name>.{val}");
        }
    }
    ...
    public void Db(string sub, params string[] args)
    {
        ...
        switch (sub)
        {
            case "" or "list":
                ...
                if (dbs.Count == 0) Info("none — start the server (omniserv start mariadb) then create one");
                foreach (var d in dbs) Ok(d);
                break;
            case "create":
            {
                if (name == "") throw new BhException("usage: omniserv db create <name> [password]");
            ...
            case "passwd":
                if (name == "" || args.Length < 2) throw new BhException("usage: omniserv db passwd <name> <password>");
                Database.SetPassword(name, args[1]);
                Ok($"set password for user '{name}'");
                break;
            case "drop":
                if (name == "") throw new BhException("usage: omniserv db drop <name>");
            ...
            case "rootpw":
                ...
                break;
            default: throw new BhException("usage: omniserv db {list|create|drop|passwd|rootpw} [name] [password]");
        }
    }
    ...
    public void Pg(string sub, params string[] args)
    {
        ...
        switch (sub)
        {
            case "" or "list":
                ...
                if (dbs.Count == 0) Info("none — start the server (omniserv start postgresql) then create one");
                foreach (var d in dbs) Ok(d);
                break;
            case "create":
                if (name == "") throw new BhException("usage: omniserv pg create <name>");
                PgDatabase.Create(name); Ok($"PostgreSQL database '{name}' ready (postgres · 127.0.0.1:{PgServer.Port})");
                break;
            case "drop":
                if (name == "") throw new BhException("usage: omniserv pg drop <name>");
                PgDatabase.Drop(name); Ok($"dropped PostgreSQL database '{name}'");
                break;
            default: throw new BhException("usage: omniserv pg {list|create|drop} [name]");
        }
    }
    ...
    public void Node(string sub, params string[] args)
    {
        ...
        var fnmArgs = sub switch
        {
            ...
            _ => throw new BhException("usage: omniserv node {list|install|use|uninstall} [version]"),
        };
        if (sub is "install" or "i" or "use" or "default" or "uninstall" or "rm" or "uni" && v == "")
            throw new BhException($"usage: omniserv node {sub} <version>");
    ...
    public void NodeSiteList()
    {
        ...
        if (!any) Info("none — omniserv nodesite add <name> --fe-dir … --fe-cmd … --fe-port …");
    }
    ...
    public void PySiteList()
    {
        ...
        if (!any) Info("none — omniserv pysite add <name> --dir … --port … [--cmd …] [--venv yes]");
    }
    ...
    public void Tunnel(string sub, params string[] args)
    {
        ...
        switch (sub)
        {
            ...
            case "start":
            {
                if (name == "") throw new BhException("usage: omniserv tunnel start <site>");
                var conf = Path.Combine(Paths.NginxSites, $"{name}.conf");
                if (!File.Exists(conf)) throw new BhException($"no site '{name}'");
                if (!Nginx.Running()) throw new BhException("nginx not running — start it first (omniserv start nginx)");
                ...
                var (ok, msg) = OmniServ.Core.Tunnel.Start(name, domain, origin);
                if (ok) { Ok($"public URL: {msg}"); Info($"origin: {origin} (Host: {domain})"); }
                else No(msg);
                break;
            }
            case "stop":
                if (name == "") throw new BhException("usage: omniserv tunnel stop <site>");
                OmniServ.Core.Tunnel.Stop(name); Ok($"tunnel stopped ({name})");
                break;
            case "url":
                if (name == "") throw new BhException("usage: omniserv tunnel url <site>");
                Out(OmniServ.Core.Tunnel.Url(name) ?? "(no tunnel)");
                break;
            case "" or "list" or "status":
                Hdr("Tunnels");
                var any = false;
                foreach (var (n, u) in OmniServ.Core.Tunnel.List()) { Ok($"{n,-16} {u}"); any = true; }
                if (!any) Info("none running — omniserv tunnel start <site>");
                break;
            default: throw new BhException("usage: omniserv tunnel {install|start <site>|stop <site>|url <site>|list}");
        }
    }

    /// <summary>Download phpMyAdmin and serve it at phpmyadmin.&lt;tld&gt; (connects to OmniServ's MySQL).</summary>
    public void PhpMyAdmin()
    {
        ...
        if (!DbServer.Running()) Warn("database not running — start it so phpMyAdmin can log in: omniserv start mariadb");
    ...
    public void Mailpit()
    {
        ...
        if (OmniServ.Core.MailpitServer.Start())
            Ok($"mailpit running — SMTP :{OmniServ.Core.MailpitServer.SmtpPort}, UI :{OmniServ.Core.MailpitServer.UiPort}");
        else { No("mailpit failed to start"); return; }

        var domain = $"mailpit.{cfg.Tld}";
        NginxConfig.RenderProxyVhost("mailpit", domain, OmniServ.Core.MailpitServer.UiPort, cfg);
        Ok($"site vhost: mailpit.conf (proxy → :{OmniServ.Core.MailpitServer.UiPort})");
        if (Nginx.Running()) Nginx.Reload(cfg);
        else { var (ok, msg) = Nginx.Start(cfg); if (ok) Ok(msg); else Warn(msg); }
        EnsureHosts(domain);

        Hdr("Mailpit ready");
        Info($"url    : http://{domain}   (or http://127.0.0.1:{OmniServ.Core.MailpitServer.UiPort})");
        Info($"SMTP   : 127.0.0.1:{OmniServ.Core.MailpitServer.SmtpPort}  — point php sendmail/SMTP here");
    }
