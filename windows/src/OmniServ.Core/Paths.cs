...
/// <summary>
/// Filesystem layout for OmniServ on Windows — the analog of the mac engine's
/// <c>~/.omniserv</c> root. Everything lives under <c>%LOCALAPPDATA%\OmniServ</c>.
/// </summary>
public static class Paths
{
    /// <summary>Root data dir, overridable via the OMNISERV_HOME env var (mirrors the mac engine).</summary>
    public static string Home =>
        Environment.GetEnvironmentVariable("OMNISERV_HOME") is { Length: > 0 } h
            ? h
            : System.IO.Path.Combine(LocalAppData(), "OmniServ");
    ...
    private static string LocalAppData()
    {
        ...
        return string.IsNullOrWhiteSpace(known) ? System.IO.Path.Combine("C:\\", "OmniServ-data") : known;
    }
    ...
    public static string ConfigJson => System.IO.Path.Combine(Config, "omniserv.json");
