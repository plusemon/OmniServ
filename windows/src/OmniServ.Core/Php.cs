...
public static class Php
{
    ...
    public static string IniPath(string version)
    {
        ...
        if (!File.Exists(ini))
        {
            ...
            else File.WriteAllText(ini, $"; OmniServ-created php.ini for {version}\n; Add your directives below.\n");
        }
        return ini;
    }
    ...
    /// ionCube loader bundle. OmniServ flattens PHP into bin\php\&lt;version&gt; (no vs## token in the path),
    ...
    public static void Ioncube(string version, Action<string> log)
    {
        ...
        else log("restart this PHP to load it: omniserv restart all");
    }
