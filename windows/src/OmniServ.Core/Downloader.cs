...
public static class Downloader
{
    ...
    // NOT omniserv.exe. That keeps antivirus behavioral scanners from flagging omniserv
    ...
    /// <summary>Extract a zip via the signed system tar.exe (so tar, not omniserv, creates the exes).
    ...
    private static void SeedPhpIni(string phpDir)
    {
        ...
        if (File.Exists(seed))
        {
            ...
            // enable the extensions OmniServ sites commonly need
    ...
    /// installer/UAC. Download with curl's DEFAULT UA (no custom OmniServ UA → avoids any CDN 403).</summary>
    ...
    /// We fetch the NON-thread-safe (nonts) bundle because OmniServ's php-cgi builds are NTS — its
    ...
    public static async Task InstallPhpMyAdmin(string root)
    {
        ...
        // config.inc.php: connect to OmniServ's MySQL as passwordless root.
