...
public static class Nginx
{
    ...
    public static (bool ok, string msg) Start(Config cfg)
    {
        ...
        if (exe is null) return (false, "nginx not installed — run: omniserv install nginx");
    ...
    public static void Stop()
    {
        ...
        // then force-kill any OmniServ nginx.exe still alive, and block until none remain.
    ...
    /// <summary>Force-kill any lingering OmniServ nginx.exe (matched by install path so an unrelated
