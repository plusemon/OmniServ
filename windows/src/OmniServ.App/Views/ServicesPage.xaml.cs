...
public sealed partial class ServicesPage : Page
{
    ...
    private async void Refresh()
    {
        ...
        {
            Key = s.Key, Version = OmniServ.Core.Services.ShortVersion(s.Key, cfg),
        ...
        Groups.ItemsSource = Order
            ...
            {
                Title = o.title,
                Rows = rows.Where(r => OmniServ.Core.Services.RoleOf(r.Key) == o.role).ToList(),
            })
    ...
    private void AutoStar_Click(object sender, RoutedEventArgs e)
    {
        ...
        var cfg = Config.Load();
        if (tb.IsChecked == true) OmniServ.Core.Services.Enable(key, cfg);
        else OmniServ.Core.Services.Disable(key, cfg);
    }
    ...
    private async void Uninstall_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Title = "Uninstall", Content = $"Remove the {key} binaries from OmniServ's bin folder?",
