...
# OmniServ — Linux / Debian platform delta layer  (apt + systemd)
# =============================================================================
# Sourced by ../engine/omniserv at the very end (just before the command dispatch)
...
#   • OmniServ runs its OWN nginx + php-fpm processes (binaries from apt, all configs
#     under ~/.omniserv) — exactly like the Mac runs brew binaries with its own config.
...
# Privileged verbs are run by the GUI via `pkexec env OMNISERV_HOME=… bash <engine> <verb>`
# (a single polkit prompt per action) or from a terminal via `sudo omniserv …`. Either way
# we then run as root but must act ON BEHALF OF the desktop user: target THEIR ~/.omniserv,
...
if [ "$(id -u)" = 0 ]; then
  # OMNISERV_OWNER_UID: set by the boot-time system unit (loginitem), where there is no
  ...
  _ru="${PKEXEC_UID:-${SUDO_UID:-${OMNISERV_OWNER_UID:-}}}"
  if [ -n "${_ru:-}" ] && [ "$_ru" != 0 ]; then
    ...
    BH_HOME="${OMNISERV_HOME:-$HOME/.omniserv}"
  fi
  SUDO=""   # already root — no nested elevation
else
  SUDO="${OMNISERV_SUDO:-sudo}"
  # From the GUI there is NO controlling terminal, so a privileged op that needs a password
  # would BLOCK on an invisible sudo/polkit prompt → the app hangs ("OmniServ Is Not
  ...
  # a terminal user piping output (`omniserv install X | tee log`) must keep the normal prompt.
  if [ "${OMNISERV_GUI:-}" = 1 ]; then SUDO="$SUDO -n"; fi
fi
...
_bh_fix_ownership(){
  ...
  case "$_BH_VERB:$_BH_SUB" in
    site:add|pysite:add|nodesite:add)
      local sr; sr="$(jget sites_root "$HOME/OmniServ/www" 2>/dev/null)"
...
# on any release. We install one to /usr/local/lib/omniserv/php/<v>/php-fpm and symlink it in at the
...
_static_php_install(){
  ...
  sysdir="/usr/local/lib/omniserv/php/$v"; tmp="$(mktemp -d)"
...
# upgrades or a previous OmniServ install) instead of failing instantly with "Could not get lock".
...
# Stop+disable the distro's own systemd unit for a service OmniServ runs itself, so its
...
cmd_install() {
  need_init
  [ $# -ge 1 ] || die "usage: omniserv install <svc|all>  (e.g. nginx, php@8.4, mariadb, redis, mkcert, dnsmasq)"
  local targets=()
  if [ "$1" = all ]; then
    ...
    case "$key" in
      php@*)
        ...
        # Fallback: a fully-static portable build — works on any release the distro/Ondřej can't cover.
          ...
          # the moment Ondřej ships packages for this release — no OmniServ update needed.
          if [ "${v%%.*}" -lt 8 ]; then
            if ls /etc/apt/sources.list.d/*ondrej*php*.bhdisabled >/dev/null 2>&1; then
              no "PHP $v needs the Ondřej PHP repo, which has no packages for Ubuntu '$(_codename)' yet. OmniServ re-checks on every install, so this will start working as soon as they publish builds — until then PHP $v needs Ubuntu 24.04/22.04, or pick PHP 8.0+ here."
            else
      ...
      mariadb|mysql)
        ...
        if _apt install -y "$pkg"; then
          ...
          _db_open_root "$key"
          $SUDO systemctl disable "$key" >/dev/null 2>&1 || true   # no autostart; OmniServ starts it on demand
          ok "$key installed"
...
cmd_update() {
  need_init; [ $# -ge 1 ] || die "usage: omniserv update <svc|all>"
  _apt_update_once
  local targets=()
  if [ "$1" = all ]; then
  ...
  for key in "${targets[@]}"; do
    ...
    # Restart OmniServ's own running instance onto the new binary so sites keep working.
    case "$role" in
...
cmd_uninstall() {
  need_init; [ $# -ge 1 ] || die "usage: omniserv uninstall <svc>"
  local key pkg
  for key in "$@"; do
    svc_exists "$key" || { warn "unknown service: $key"; continue; }
    svc_installed "$key" || { warn "$key not installed"; continue; }
    ...
    if [[ "$key" == php@* ]] && _is_static_php "${key#php@}"; then
      ...
      $SUDO rm -rf "/usr/local/lib/omniserv/php/${key#php@}"
...
# OmniServ runs nginx + php-fpm itself; everything else is a distro daemon we toggle.
_systemd_unit(){
  case "$1" in
...
[Unit]
Description=Mailpit (OmniServ mail catcher)
...
# cloudflared (Cloudflare quick tunnels). Installed under the USER-owned config root (~/.omniserv/bin),
...
cloudflared_install(){
  ...
  if curl $_CURL_HTTPS -fsSL "$url" -o "$tmp" && [ -s "$tmp" ]; then
    install -m 0755 "$tmp" "$bin"; rm -f "$tmp"   # ~/.omniserv is user-owned → no sudo
...
db_ext_ensure(){
  ...
  if _php_has_mysqli "$want"; then printf 'php@%s\n' "$want"; return 0; fi
  {
    if _is_static_php "$want"; then
      ...
      $SUDO rm -f "/usr/sbin/php-fpm$want" "/usr/local/lib/omniserv/php/$want/php-fpm" 2>/dev/null || true
...
# Resolve a brew formula OR a OmniServ service key to its systemd unit name.
_unit_for(){
  case "$1" in
...
# dnsmasq status on Linux. OmniServ-Linux resolves *.test via a managed /etc/hosts block by
...
cmd_doctor() {
  ...
  systemctl is-active  --quiet systemd-resolved 2>/dev/null && info "systemd-resolved owns 127.0.0.53:53 — OmniServ uses /etc/hosts for *.test by default (wildcard dnsmasq is opt-in)"
}
...
# managed block in /etc/hosts — one line per OmniServ domain, rewritten from the live
...
HOSTS_BEGIN="# OMNISERV-START (managed — do not edit; rewritten on site changes)"
HOSTS_END="# OMNISERV-END"
...
maybe_reload_nginx(){
  ...
  if nginx_running; then
    ...
    nginx_start >/dev/null 2>&1 && ok "nginx started" || warn "start nginx to serve the site (omniserv start nginx)"
  fi
}
...
nginx_reload(){
  ...
  if $pre "$bin" -s reload -c "$BH_HOME/nginx/nginx.conf" -p "$BH_HOME/nginx" 2>/dev/null; then
    ...
    nginx_restart >/dev/null 2>&1 && ok "nginx restarted" || warn "reload failed — run: omniserv restart nginx"
  fi
}

# `omniserv dns` — default: (re)sync /etc/hosts for all sites. `omniserv dns wildcard` —
...
cmd_dns() {
  ...
  case "$mode" in
    hosts|"")
      ...
      hosts_sync_all
      ok "all OmniServ domains point to 127.0.0.1 via /etc/hosts"
      info "for true wildcard *.$(jget tld test): omniserv dns wildcard" ;;
    wildcard)
      hdr "DNS: wildcard *.$(jget tld test) via dnsmasq + systemd-resolved"
      svc_installed dnsmasq || die "dnsmasq not installed — run: omniserv install dnsmasq"
      ...
      printf 'no-resolv\nlisten-address=127.0.0.54\nbind-interfaces\naddress=/%s/127.0.0.1\n' "$tld" \
        | $SUDO tee /etc/dnsmasq.d/omniserv.conf >/dev/null
      ...
      printf '[Resolve]\nDNS=127.0.0.54\nDomains=~%s\n' "$tld" \
        | $SUDO tee /etc/systemd/resolved.conf.d/omniserv.conf >/dev/null
    ...
    *) die "usage: omniserv dns [hosts|wildcard]" ;;
  esac
}
...
# to the desktop user via OMNISERV_OWNER_UID), and a user autostart .desktop shows the tray at
...
_loginitem_unit(){ echo /etc/systemd/system/omniserv.service; }
_loginitem_autostart(){ echo "$HOME/.config/autostart/com.emon.omniserv-tray.desktop"; }
loginitem_enabled(){ systemctl is-enabled omniserv.service >/dev/null 2>&1; }
...
_loginitem_retire_user_unit(){
  if [ "$(id -u)" = 0 ] && [ -n "${USER_NAME:-}" ] && [ "$USER_NAME" != root ]; then
    XDG_RUNTIME_DIR="/run/user/$(id -u "$USER_NAME")" runuser -u "$USER_NAME" -- \
      systemctl --user disable omniserv.service >/dev/null 2>&1 || true
  else
    systemctl --user disable omniserv.service >/dev/null 2>&1 || true
  fi
  rm -f "$HOME/.config/systemd/user/omniserv.service" \
        "$HOME/.config/systemd/user/default.target.wants/omniserv.service" 2>/dev/null || true
...
cmd_loginitem(){
  ...
  local engine="${_bh_engine_dir:-$(dirname "$(readlink -f "$0")")}/omniserv"
  case "$sub" in
    enable)
      [ "$(id -u)" = 0 ] || die "start-at-login needs admin — run: sudo omniserv loginitem enable"
      _loginitem_retire_user_unit
      # bake the desktop user's uid into the unit: at boot there is no PKEXEC_UID/SUDO_UID,
      # so the engine reads OMNISERV_OWNER_UID to run workers/sockets as the user (not root).
      local owner_uid; owner_uid="${_ru:-$(id -u "${USER_NAME:-$(id -un)}")}"
      cat > "$unit" <<UNIT
[Unit]
Description=OmniServ — start local web services at boot
After=network.target
...
Type=oneshot
RemainAfterExit=yes
Environment=OMNISERV_OWNER_UID=$owner_uid
Environment=OMNISERV_HOME=$BH_HOME
...
UNIT
      ...
      systemctl enable omniserv.service >/dev/null 2>&1 && ok "start-at-boot enabled (system service)" || warn "couldn't enable the system service"
...
Type=Application
Name=OmniServ (tray)
Comment=Show the OmniServ tray icon at login
Exec=omniserv-gui --background
...
DESK
      ...
      ;;
    disable)
      [ "$(id -u)" = 0 ] || die "start-at-login needs admin — run: sudo omniserv loginitem disable"
      ...
      systemctl disable omniserv.service >/dev/null 2>&1 || true
    ...
    *) die "usage: omniserv loginitem {enable|disable|status}" ;;
  esac
}
...
helper_install(){
  ...
  tmp="$(mktemp)"
  cat > "$tmp" <<SUDO
# OmniServ — let $u start/stop/reload nginx (binds :80/:443) without a password.
# Remove with: sudo omniserv helper uninstall
...
mysql_run(){
  ...
  elif [ -n "${OMNISERV_OLD_DB_PASSWORD:-}" ] && MYSQL_PWD="$OMNISERV_OLD_DB_PASSWORD" "$c" -u root -N -e "SELECT 1;" >/dev/null 2>&1; then MYSQL_PWD="$OMNISERV_OLD_DB_PASSWORD" "$c" -u root "$@"
...
db_secure_bind(){
  ...
  printf '# OmniServ — keep the DB on localhost only (root has no password)\n[mysqld]\nbind-address = 127.0.0.1\n' \
    | $SUDO tee "$f" >/dev/null 2>&1 || true
}

# ── self-update (CLI convenience) ─────────────────────────────────────────────
# Fetch + install the newest OmniServ .deb straight from GitHub releases, so terminal users don't have
...
cmd_self_update(){
  ...
  if ! curl $_CURL_HTTPS -fsSL "$url" -o "$tmp/omniserv.deb"; then
  ...
  $SUDO env DEBIAN_FRONTEND=noninteractive dpkg -i "$tmp/omniserv.deb" >/dev/null 2>&1 || true
  $SUDO env DEBIAN_FRONTEND=noninteractive apt-get -f install -y >/dev/null 2>&1 || true
  rm -rf "$tmp"
  local now; now="$(dpkg-query -W -f='${Version}' omniserv 2>/dev/null || true)"
  if [ "$now" = "$latest" ]; then
    ok "updated ${cur:-?} → $latest — restart the OmniServ app if it's open"; return 0
  fi
  no "self-update failed (dpkg/apt error) — grab the .deb from the releases page and run: sudo dpkg -i omniserv_${latest}_all.deb && sudo apt-get -f install -y"; return 1
}
...
# dir, which PHP reads BEFORE our PHP_INI_SCAN_DIR extra dir — so an entry in OmniServ's conf.d would
# always load second and abort. 00-omniserv-ioncube.ini in the same distro dir sorts before 10-opcache.
...
php_ioncube(){
  ...
  while read -r key formula probe role; do
    ...
    for sapi in fpm cli; do
      ...
      printf 'zend_extension=%s\n' "$so" | $SUDO tee "/etc/php/$mm/$sapi/conf.d/00-omniserv-ioncube.ini" >/dev/null && wrote=1
    done
    # Retire any broken entry the pre-fix code wrote into OmniServ's own conf.d (a dar .so there
  ...
  info "restart PHP-FPM to load it: omniserv restart all  (verify: omniserv php status)"
}

php_status(){
  # Same as the shared version, but "configured" is detected at the DISTRO conf.d path this
  # platform writes to (the shared version only looks in OmniServ's own conf.d).
  local IFS='|' key formula probe role
  hdr "PHP versions"
  while read -r key formula probe role; do
    [ "$role" = php ] || continue
    ...
    [ -f "/etc/php/$mm/fpm/conf.d/00-omniserv-ioncube.ini" ] && ic="yes"
...
# native .htaccess support and forwards PHP to OmniServ's EXISTING php-fpm pools over their unix
...
# is fatal to us: it holds *:7080/*:8088 sockets that make the first OmniServ-config (re)start
...
ols_start(){
  ols_installed || { no "OpenLiteSpeed not installed — omniserv install openlitespeed"; return 1; }
...
# .htaccess watcher, rewrites the server config as a OmniServ-managed file, and installs the
...
ols_install(){
  ...
  # don't rely on the repo bootstrapper having pulled it. (Sites still use OmniServ's php-fpm.)
...
# One-time (idempotent) OmniServ configuration of a fresh OLS install:
...
#    OMNISERV-SITES marker block that _ols_sync_config regenerates
...
_ols_configure(){
  ...
  if ! $SUDO grep -q "OMNISERV-SITES-BEGIN" "$conf" 2>/dev/null; then
    ...
    $SUDO cp "$conf" "$conf.omniserv-orig" 2>/dev/null || true
    local tmp; tmp="$(mktemp)"
    cat > "$tmp" <<OLSCONF
# OmniServ-managed OpenLiteSpeed config (package original: httpd_config.conf.omniserv-orig).
# Site vhosts + the loopback listener live between the OMNISERV-SITES markers below and are
# REGENERATED by OmniServ on every site change — edit anything else freely.
serverName                omniserv
user                      $(_bh_site_user)
...
# OMNISERV-SITES-BEGIN
# OMNISERV-SITES-END
OLSCONF
    $SUDO cp "$tmp" "$conf"; rm -f "$tmp"
    ok "OpenLiteSpeed config now OmniServ-managed (loopback :$OLS_PORT, workers run as $(_bh_site_user))"
  fi
  # Admin console: loopback only. (Password stays whatever the package generated — see
  # $LSWS_ROOT/adminpasswd if it wrote one; the console is optional for OmniServ.)
...
_ols_watcher_install(){
  local script=/usr/local/lib/omniserv/ols-htaccess-watch.sh
  local unit=/etc/systemd/system/omniserv-ols-watch.service
  local sroot; sroot="$(jget sites_root "$HOME/OmniServ/www")"
  ...
  if [ -f "$script" ] && grep -q "OMNISERV-OLSWATCH-V3" "$script" 2>/dev/null \
     && systemctl is-active --quiet omniserv-ols-watch 2>/dev/null; then return 0; fi
  $SUDO mkdir -p /usr/local/lib/omniserv
  $SUDO tee "$script" >/dev/null <<'WATCH'
#!/bin/bash
# OMNISERV-OLSWATCH-V3
# OmniServ: graceful-restart OpenLiteSpeed when any site's .htaccess changes.
...
WATCH
  ...
  $SUDO tee "$unit" >/dev/null <<UNIT
[Unit]
Description=OmniServ OpenLiteSpeed .htaccess watcher
After=network.target
...
UNIT
  $SUDO systemctl daemon-reload >/dev/null 2>&1 || true
  $SUDO systemctl enable omniserv-ols-watch >/dev/null 2>&1 || true
  # restart (not just enable --now): a running OLD copy of the script must be replaced
  $SUDO systemctl restart omniserv-ols-watch >/dev/null 2>&1 || true
...
render_ols_vhost(){
  ...
  local label sock d="$LSWS_ROOT/conf/vhosts/omniserv-$1"
  label="$(php_label "$phpkey")"; sock="$(php_sock "$phpkey")"
  $SUDO mkdir -p "$d"
  $SUDO tee "$d/vhconf.conf" >/dev/null <<OLSVH
# OmniServ OLS vhost: $name ($domain) php=$phpkey — regenerated on site changes
docRoot                   $root
vhDomain                  $domain
enableGzip                1
...
# Heal OLS vhosts written before the HTTPS-forwarding rule (parity #10): re-render any omniserv OLS
...
_ols_heal_vhosts(){
  ...
  for f in "$BH_HOME"/nginx/sites/*.conf; do
    ...
    vh="$LSWS_ROOT/conf/vhosts/omniserv-$name/vhconf.conf"
...
render_nginx_ols_proxy_vhost(){
  ...
  cat > "$conf" <<NGINX
# OmniServ site: $name  ($domain)  php=$phpkey server=ols
server {
...
# Regenerate the OMNISERV-SITES block in httpd_config.conf from the site list (every vhost with
...
_ols_sync_config(){
  ...
  block="$(mktemp)"
  {
    ...
    for f in "$BH_HOME"/nginx/sites/*.conf; do
      ...
      root="$(vhost_root_read "$f")"
      printf 'virtualhost omniserv-%s {\n  vhRoot                  %s\n  configFile              conf/vhosts/omniserv-%s/vhconf.conf\n  allowSymbolLink         1\n  enableScript            1\n  restrained              0\n}\n' "$name" "$root" "$name"
      maps="$maps  map                     omniserv-$name $domain
"
    done
    if [ -n "$maps" ]; then
      printf 'listener OmniServ {\n  address                 127.0.0.1:%s\n  secure                  0\n%s}\n' "$OLS_PORT" "$maps"
    fi
    echo "# OMNISERV-SITES-END"
  } > "$block"
  local cur new; cur="$(mktemp)"; new="$(mktemp)"
  $SUDO cat "$conf" > "$cur"
  awk -v bf="$block" '
    /# OMNISERV-SITES-BEGIN/ { skip=1; while ((getline line < bf) > 0) print line; next }
    /# OMNISERV-SITES-END/   { skip=0; next }
    !skip { print }
  ' "$cur" > "$new"
  grep -q "OMNISERV-SITES-BEGIN" "$new" || cat "$block" >> "$new"
  $SUDO cp "$new" "$conf"
  rm -f "$block" "$cur" "$new"
}
...
site_set_server() {
  [ $# -ge 2 ] || die "usage: omniserv site server <name> <nginx|apache|ols>"
  ...
  [ "$newsrv" != apache ] || svc_installed httpd || die "apache needs httpd — omniserv install httpd"
  ...
  if [ "$oldsrv" = ols ] && [ "$newsrv" != ols ]; then
    $SUDO rm -rf "$LSWS_ROOT/conf/vhosts/omniserv-$name" 2>/dev/null || true
  fi
...
site_rm(){
  ...
  if [ "$_was_ols" = 1 ]; then
    $SUDO rm -rf "$LSWS_ROOT/conf/vhosts/omniserv-$_name" 2>/dev/null || true
    _ols_apply
  fi
  return $rc
}

# start/stop: `omniserv start|stop ols`, OLS joins `start all` (when it has sites) + `stop all`.
...
render_apache_main() {
  ...
  cat > "$conf" <<APACHE
# Generated by OmniServ (Linux) — apache2 backend behind nginx. Do not edit by hand.
...
<VirtualHost 127.0.0.1:$APACHE_PORT>
    ServerName omniserv-default.invalid
    <Location "/">
        Require all denied
    </Location>
</VirtualHost>
