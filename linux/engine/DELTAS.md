# Engine deltas — exactly what to change in `../../engine/omniserv` for Linux

The macOS engine (`engine/omniserv`, bash) runs on Linux **almost unchanged** — same php-fpm + unix
...
2. **systemd-resolved drop-in** for true wildcard `*.test`: run dnsmasq on a private address +
   `/etc/systemd/resolved.conf.d/omniserv.conf` (or `resolvectl` `~test` routing domain) → opt-in toggle.
3. **NetworkManager dnsmasq** drop-in `/etc/NetworkManager/dnsmasq.d/omniserv.conf` (`address=/test/127.0.0.1`)
   when NM manages DNS.
...
- **`~/Library/LaunchAgents/com.emon.omniserv.login.plist`** + **`launchctl bootout/bootstrap/enable`** →
  replace with a **systemd `--user` service** (`~/.config/systemd/user/omniserv.service`,
  `systemctl --user enable omniserv`, + `loginctl enable-linger $USER` for pre-login), **or** a
  `~/.config/autostart/omniserv.desktop`. The `loginitem_enabled` check in the api snapshot maps to
  `systemctl --user is-enabled omniserv`.

## 4. Elevation — osascript → pkexec / sudoers

- The DNS/cert/hosts writes are invoked **privileged**. The comment at `cmd_dns` mentions
  "osascript admin"; on Linux wrap privileged verbs with **`pkexec /path/to/omniserv <verb>`**
  (PolicyKit GUI prompt — the direct analog of `osascript … with administrator privileges`).
- **`helper install`** already writes **`/etc/sudoers.d/omniserv`** (cross-platform!) — keep it for the
...
- **`~/.omniserv`** data layout, the **`api` JSON** shape, the **`pysite`/`nodesite`** config JSONs
- **MariaDB 127.0.0.1 bind** security fix — same `my.cnf.d/omniserv.cnf` idea (path differs per source)
...
- Writes `zend_extension=` to **`/etc/php/<mm>/{fpm,cli}/conf.d/00-omniserv-ioncube.ini`** (sudo),
  NOT OmniServ's own conf.d: ionCube must load before opcache or it aborts, and the distro's
  ...
  `00-ioncube.ini` the pre-fix code left in OmniServ's conf.d.
...
the exact shape of the apache backend (:8080). PHP goes from OLS to OmniServ's EXISTING php-fpm
...
- Takeover: `_ols_kill` (unit stop + lswsctrl stop + pkill; the package postinst starts OLS
  ...
  instance holding `*:8088`/`*:7080`, which makes the first OmniServ start die with "address in
  use"). Then httpd_config.conf is rewritten as a OmniServ-managed file (original backed up),
  loopback listeners only, workers as the site user (fpm-socket + file access), site block
  between `# OMNISERV-SITES-*` markers regenerated by `_ols_sync_config` on every site change.
...
- .htaccess: per-vhost `autoLoadHtaccess 1` + the `omniserv-ols-watch` systemd unit —
  ...
  script carries a `OMNISERV-OLSWATCH-V<N>` marker; `_ols_apply` re-deploys it when the version
...
solved via the DISTRO conf.d (`/etc/php/<mm>/{fpm,cli}/conf.d/00-omniserv-ioncube.ini` sorts before
...
The shared `engine/omniserv` already has the full subdomain feature (`site_subdomain`, `vhost_aliases`,
conflict detection, SSL reissue — verified on macOS v1.7.9), so **Linux needs NO engine override**. The
Linux GTK GUI is also already on master (`linux/app/omniserv/pages.py`: `site_subdomains` dialog +
